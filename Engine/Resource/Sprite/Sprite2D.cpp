...
#include "Resource/Texture/D2DTexture.h"

namespace Engine
{
    Sprite2D::Sprite2D(const std::wstring& _key, const std::wstring& _path, ResourceType _type, SpriteData _data)
        : Resource(_key, _path, _type)
    {
        ...
        HRESULT hr;
        IWICImagingFactory* imgFactory = D2DRenderSystem::GetIMGFactory();
        ...
        hr = D2DRenderSystem::GetRenderTarget()->CreateBitmapFromWicBitmap(converter, nullptr, &tempTexture);
        if (FAILED(hr)) assert(false);
        ...
        for (int y = 0; y < rows; ++y)
        {
            for (int x = 0; x < cols; ++x)
            {
                ...
                hr = D2DRenderSystem::GetRenderTarget()->CreateBitmap(
                ...
                hr = D2DRenderSystem::GetRenderTarget()->CreateCompatibleRenderTarget(D2D1::SizeF(rect.right - rect.left, rect.bottom - rect.top), &bitmapRenderTarget);
                if (FAILED(hr))
                    assert(false);
