...
namespace Engine
{
	...
	bool Animator2D::Draw(Camera2D* _camera)
	{
		...
		Rect_F dstRect(
			renderOffset.x + data.offset.x,
			renderOffset.y + data.offset.y,
		...
		//if (Debug::IsActive())
		//{
		//	ID2D1SolidColorBrush* brush = D2DRenderSystem::GetBrush();
	...
	void Animator2D::AddAnimation(std::string _key, Animation2D* _ani)
	{
		if (_ani == nullptr)
			assert(false);
	...
	Animation2D* Animator2D::FindAnimation(const std::string& _key)
	{
	...
	{
		Animation2D* _ani = FindAnimation(_key);
		if (_ani == nullptr)
			assert(false);
